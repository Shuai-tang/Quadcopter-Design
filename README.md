...
本仓库记录了面向科研与教学用途的四旋翼（Quadrotor）飞行器实验平台的硬件选型与系统搭建过程。平台采用模块化航电架构，涵盖飞控、电调、无刷电机、螺旋桨及视觉/计算单元，适用于自主导航、视觉惯性里程计（VIO）、路径规划等方向的研究与验证。

---

## 1. 引言

四旋翼飞行器因其结构简单、机动性强、易于建模与控制，已成为无人系统与机器人领域常用的实验载体。本平台以**可复现、可扩展**为设计原则，选用成熟商用部件构建航电系统，便于在实验室环境下开展控制算法、状态估计与感知算法的对比与验证。
...
| 层级     | 功能           | 主要硬件                     |
|----------|----------------|------------------------------|
| 感知     | 视觉、深度、姿态 | 深度相机、USB 摄像头         |
| 计算/决策 | 算法运行、接口扩展 | 嵌入式计算机 / 主控箱        |
| 控制     | 姿态解算、导航  | 飞控（V5+ 级）               |
| 执行     | 推力与姿态实现  | 电调、无刷电机、三叶桨       |
...
本平台选用 **V5+ 级飞控**（Pixhawk 系列或兼容方案），主要特性包括：

- **主控模块**：银色金属外壳，顶面标有 “V5+” 与 “FRONT” 方向标识，通过载板固定于黑色 PCB 上。
- **接口**：  
