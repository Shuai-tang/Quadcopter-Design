# 多旋翼总体设计一览表

---

## 系统架构概览

<div align="center">

<table align="center" style="margin: 0 auto; text-align: center">
  <thead>
    <tr><th>系统</th><th>组件</th><th>具体型号</th></tr>
  </thead>
  <tbody>
    <tr>
      <td rowspan="3"><strong>动力系统</strong></td>
      <td>电机</td>
      <td>Axisflying 酷飞 AE2207 V2</td>
    </tr>
    <tr>
      <td>螺旋桨</td>
      <td>Axisflying 酷飞 BB39 桨叶黑鸟联名 V2 三叶桨</td>
    </tr>
    <tr>
      <td>电调</td>
      <td>Axisflying 酷飞 F405 飞塔（8 位 60A 四合一）</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>传感系统</strong></td>
      <td>光流测距仪</td>
      <td>micoair MTF-01</td>
    </tr>
    <tr>
      <td>深度相机</td>
      <td>Intel RealSense D430 / D415 / D435 / D435i</td>
    </tr>
    <tr>
      <td>广角单目相机</td>
      <td>待选型</td>
    </tr>
    <tr>
      <td><strong>控制系统</strong></td>
      <td>飞控</td>
      <td>CUAV 雷迅 V5+</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>通信系统</strong></td>
      <td>接收机</td>
      <td>待补充</td>
    </tr>
    <tr>
      <td>WIFI 模块</td>
      <td>待补充</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>供电系统</strong></td>
      <td>电池</td>
      <td>格氏 ACE 3300 mAh 6S 30C</td>
    </tr>
    <tr>
      <td>飞控降压模块</td>
      <td>待补充</td>
    </tr>
    <tr>
      <td>机载电脑降压模块</td>
      <td>待补充</td>
    </tr>
    <tr>
      <td><strong>计算系统</strong></td>
      <td>机载电脑</td>
      <td>待补充</td>
    </tr>
  </tbody>
</table>

</div>

---

## 1. 动力系统

### 1.1 电机

- **型号**：Axisflying 酷飞 AE2207 V2（1960KV）
- **适用**：FPV 穿越机 5 寸无人机
- **主要参数**：

| 参数 | 规格 |
|------|------|
| KV 值 | 1750 / 1960 |
| 槽极数 | 12N14P |
| 电机尺寸 | Ø27.5 × 33.2 mm |
| 轴径 | 4 mm，螺纹 M5，螺纹段长 13.8 mm |
| 安装孔 | 4-M3，螺栓圆 Ø16 mm |
| 工作电压 | 6S |
| 最大功率 | 771.9 W |
| 最大拉力 | 1634 g |
| 最大电流 | 32.46 A |
| 内阻 | 71.53 mΩ |
| 空载电流(10V) | 0.89 A |
| 硅胶线 | 20# 150 mm |
| 重量(含线) | 35.2 g |
- **购买链接**：[Axisflying酷飞AE2207 V2 FPV穿越机5寸无人机航模无刷电机马达 - 淘宝网](https://s.taobao.com/search?q=Axisflying%E9%85%B7%E9%A3%9EAE2207+V2+FPV%E7%A9%BF%E8%B6%8A%E6%9C%BA)

| 实物 | 尺寸图 | 
|------|--------|
| ![电机](./media/motor.jpg) | ![电机尺寸](./media/motor_dims.png) | 

### 1.2 电调

- **型号**：Axisflying 酷飞 F405 飞塔（含 F405 飞控）
- **规格**：8 位 60A 电调四合一
- **购买链接**：[Axisflying酷飞FPV穿越机飞塔F405飞控无人机8位60A电调四合一 - 淘宝网](https://s.taobao.com/search?q=Axisflying%E9%85%B7%E9%A3%9EFPV%E7%A9%BF%E8%B6%8A%E6%9C%BA%E9%A3%9E%E5%A1%94F405)

飞控接线示意（正面/反面）：

| 正面 | 反面 |
|------|------|
| ![飞控接线正面](./media/fc_wiring_front.png) | ![飞控接线反面](./media/fc_wiring_back.png) |

*接线包含：摄像头 CAM1/CAM2、四合一电调 M1–M8、GPS、模拟图传、接收机 TBS/Elrs/SBUS、LED、蜂鸣器、HD 系统等。*

### 1.3 螺旋桨

- **型号**：Axisflying 酷飞 BB39 桨叶黑鸟联名 V2 三叶桨
- **适用**：FPV 穿越机 5 寸螺旋桨
- **主要参数**（来自文档附图）：
  - 尺寸：4.9 英寸；桨盘直径：126 mm；最大桨叶宽度：14.6 mm
  - 螺距：2.6；材质：PC；孔径：5 mm；桨毂厚度：6 mm
  - 重量：3.7 g
  - 颜色：透明蓝 / 透明橙 / 透明灰
  - 推荐搭配电机：BB 2207 V3 / AE 2207 V2
- **购买链接**：[Axisflying酷飞BB39桨叶黑鸟联名V2三叶桨FPV穿越机5寸螺旋桨叶片 - 淘宝网](https://s.taobao.com/search?q=Axisflying%E9%85%B7%E9%A3%9EBB39%E6%A1%A3%E5%8F%B6)

![三叶螺旋桨](./media/propeller.jpg)  
*图：三叶桨实物。*

**螺旋桨规格参数**

| 参数 | 规格 |
|------|------|
| 尺寸 | 4.9 英寸 |
| 重量 | 3.7 g |
| 螺距 | 2.6 |
| 材质 | PC |
| 孔径 | 5 mm |
| 桨毂厚度 | 6 mm |
| 桨盘直径 | 126 mm |
| 最大桨叶宽度 | 14.6 mm |
| 颜色 | 透明蓝 / 透明橙 / 透明灰 |
| 推荐搭配电机 | BB 2207 V3 / AE 2207 V2 |

---

## 2. 传感系统

### 2.1 光流测距仪

- **型号**：micoair MTF-01（光流 + 激光测距一体）
- **购买链接**：[micoair MTF-01光流8米激光测距一体模组定位PMW3901传感器无人机 - 淘宝网](https://s.taobao.com/search?q=MTF-01%E5%85%89%E6%B5%81%E6%B5%8B%E8%B7%9D%E4%BB%AA)

**MTF-01 规格参数**

| 分类 | 参数 | 规格 |
|------|------|------|
| **外形** | 产品重量 | 4.5 g |
| | 产品尺寸 | 29×16.5×15 mm |
| **输出** | 输出方式 | UART |
| | 刷新频率 | 100 Hz |
| **测距** | 测距范围 | 0.01–8 m |
| | 测距盲区 | 1 cm |
| | 测距精度 | 2% |
| | 测距 FOV | 6° |
| **光流** | 光流 FOV | 42° |
| | 光流环境光需求 | >60 Lux |
| | 光流工作距离 | >80 mm |
| **光学** | 波长 | 830–870 nm |
| | 抗环境光 | 70K Lux 照度 |
| **电气** | 功耗 | 500 mW |
| | 工作电压 | 4.0–5.5 V |
| **环境** | 工作温度 | -10°C–60°C |

| 模块实物（示例） |
|------------------|
| ![光流模块](./media/optical_flow_module.png) |

### 2.2 深度相机

- **型号**：英特尔 Intel RealSense D430+RGB / D415 / D435 / D435i
- **购买链接**：[英特尔 Intel D415 D435 D435i深度实感相机 双目立体实感相机 - 淘宝网](https://s.taobao.com/search?q=Intel+D435+%E6%B7%B1%E5%BA%A6%E7%9B%B8%E6%9C%BA)

**D430+RGB 规格参数**

| 分类 | 参数 | 规格 |
|------|------|------|
| **感应** | 感应技术 | 主动红外立体声（全球快门） |
| | 使用环境 | 室内/室外 |
| **深度/IR** | 最高分辨率 | 1280×720 |
| | 帧率 | 最高 90 fps |
| | 最近距离 | 0.3 m |
| | 最远距离 | 约 10 m（视校准、场景与光照而定） |
| **RGB** | 分辨率与帧率 | 1920×1080 @ 30 fps |
| **视场角** | FOV | 69.4°×42.5°×77° (±3°) |
| **接口** | 连接 | USB 3.0 Type-C |
| **安装** | 安装点 | 1× 1/4-20 UNC 螺纹，2× M3 螺纹 |
| **尺寸** | 整机/模块 | 91.2×65.5×100.6 mm / 90×25×25 mm（模块） |

| 深度相机实物 | 
|--------------|
| ![深度相机](./media/depth_camera.png) | 

### 2.3 广角单目相机

- **状态**：Todo（待选型）

---

## 3. 控制系统

### 3.1 飞控

- **型号**：CUAV 雷迅 V5+
- **规格**：智能控制器开源飞控，兼容 Pixhawk / APM / PX4
- **购买链接**：[CUAV雷迅V5+智能控制器开源飞控pixhawk无人机船APM PX4 - 天猫](https://list.tmall.com/search_product.htm?q=CUAV%E9%9B%B7%E8%BF%85V5%2B)

**V5+ 规格参数**

| 分类 | 参数 | 规格 |
|------|------|------|
| **硬件** | 主处理器 | STM32F765（32 位 Arm® Cortex®-M7，216 MHz，2 MB Flash，512 KB RAM） |
| | 协处理器 | STM32F100（32 位 Cortex®-M3，24 MHz，8 KB SRAM） |
| **传感器** | 加速计/陀螺仪 | ICM-20602 / ICM-20689 / BMI055 |
| | 电子罗盘 | IST8310 |
| | 气压计 | MS5611×2 |
| **接口** | UART | 5 |
| | I2C | 4 |
| | SPI | 1 |
| | CAN | 2 |
| | ADC | 2 |
| | PWM 输出 | 14 |
| | PWM 输入 | 6 |
| | PPM 输入 | 1 |
| | DSM/SBUS/RSSI | 1 |
| | 电源 1/2 | 各 1 |
| | 安全开关 | 1 |
| | 蜂鸣器 | 1 |
| | USB-Type-C | 1 |
| | TF 卡槽 | 1 |
| **支持功能** | 固件 | PX4 / ArduPilot |
| | 机型 | 多旋翼 / 固定翼 / 直升机 / VTOL / 无人船 / 无人车等 |
| | 电源 | 三路电源输入冗余 |
| **尺寸与重量** | 尺寸 | 85.5×42×33 mm |
| | 重量 | 91 g |

| 飞控实物 |
|----------|
| ![CUAV V5+](./media/fc_v5_photo.jpeg) |

---

## 4. 通信系统

- 接收机
- WIFI 模块

*具体型号与购买链接待补充*

---

## 5. 供电系统

### 5.1 电池

- **品牌**：格氏 ACE（格氏正品）
- **规格**：1800/2200/3300/4000/5300/6000 mAh，4S/6S 动力锂电池；高倍率放电
- **示例**（来自文档附图）：GS33004S30 — 3300 mAh、4S1P、14.8 V、30C；XT60 主电源接口 + JST-XH 平衡头
- **购买链接**：[格氏ACE1800/2200/3300/4000/5300/6000mah4S/6S动力锂电池穿越机 - 淘宝网](https://s.taobao.com/search?q=%E6%A0%BC%E6%B0%8FACE%E7%A6%BB%E7%94%B5%E6%B1%A0)

![格氏电池示例](./media/battery.jpeg)

### 5.2 降压模块

*具体型号与购买链接待补充*

---

## 6. 计算系统

- **机载电脑**：用于路径规划、图像处理与高级控制；小型化主机（类 NUC 形态），前置 USB、网口、电源键等
- *具体型号与购买链接待补充*

![机载电脑示例](./media/onboard_pc.jpeg)

---

## 使用与复现说明

1. **硬件清单**：可依据本文档各章节采购对应组件。
2. **固件与调参**：飞控需刷写 PX4 或 ArduPilot 等固件，并完成传感器校准、电调校准与电机转向、混控配置。
3. **安全**：首次上电与试飞应在空旷、安全场地进行，螺旋桨旋转时保持安全距离。

---

*本文档随硬件迭代可能更新；具体型号与参数以实际器件与厂商说明为准。*
